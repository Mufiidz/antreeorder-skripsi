...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  CartBloc() : super(const CartState([])) {
    on<InitCart>((event, emit) {
      ...
      orders = event.orders;
      emit(state.copyWith(orders: orders, status: StatusState.idle));
    });
    ...
    on<ItemQuantity>((event, emit) {
      ...
      emit(state.copyWith(orders: newOrders, status: StatusState.idle));
    });

    on<DeleteItemCart>((event, emit) {
      emit(state.copyWith(status: StatusState.loading));
      state.data.remove(event.order);
      emit(state.copyWith(orders: state.data, status: StatusState.idle));
    });
  }
}
